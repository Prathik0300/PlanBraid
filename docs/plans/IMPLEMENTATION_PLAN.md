...
| Theme-safe integration review actions and automatic review reveal: primary integration buttons have explicit high-contrast dark/light palettes, and opening or refreshing review smoothly scrolls the modal body to the rendered candidates | `app/integrations-dialog.tsx`, `app/globals.css`, `tests/rendered-html.test.mjs` |
| Public privacy policy and integration disclosure: a responsive `/privacy` route documents account, MCP, repository, Jira, Basecamp, Slack, OAuth, retention, rights, security, and international-transfer practices; sign-in and account surfaces link to it, and deployment-owned legal/contact identity is configurable without code changes | `app/privacy/`, `app/auth-screen.tsx`, `app/planbraid-app.tsx`, `.env.example`, `tests/rendered-html.test.mjs` |
| Project-scoped team ownership: Basecamp project people and Jira project-assignable users sync into a provider-neutral participant directory; imported tasks retain stable one-or-many owner identities; the work-item drawer exposes a multi-owner dropdown; the team manager summarizes active/done workload; locally-created projects can manage assignment-only members without granting account access or writing anything back to providers | `db/setup.ts`, `lib/contracts.ts`, `lib/integrations/{basecamp,jira,service,types}.ts`, `lib/store.ts`, `app/planbraid-app.tsx`, `app/globals.css`, `tests/{integrations,project-management}.test.mjs` |
...
| **Multi-user authentication / team RBAC** | Assignment participants now exist, but they deliberately do not grant login access, permissions, or collaboration rights. Full multi-user authorization remains out of scope until there is a second signed-in user |
