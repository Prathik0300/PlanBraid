...
export function PlanbraidApp() {
  ...
  return (
    <main className={`app-shell ${sidebarOpen ? "sidebar-open" : ""}`}>
      ...
      {plan && <PlanDialog plan={plan} close={() => setPlan(null)} onItem={(id) => { setPlan(null); setSelectedItemId(id); }} />}
...
function PlanDialog({ plan, close, onItem }: { plan: ExecutionPlan; close: () => void; onItem: (workItemId: string) => void }) {
  return <div className="dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
    <section className="plan-dialog" role="dialog" aria-modal="true" aria-labelledby="plan-title">
      ...
      {plan.stuck.length > 0 && <div className="plan-stuck"><h3>Needs manual attention</h3><p className="muted">Finishing other tracked work can't unblock these; a person has to look.</p>{plan.stuck.map((item) => <button className="plan-step-row stuck" key={item.id} onClick={() => onItem(item.id)}>
        <span className={`priority ${item.priority}`} />
        <span><strong>{item.itemKey} {item.title}</strong><small>{item.reason}</small></span>
      </button>)}</div>}
    </section>
  </div>;
}
...
function TaskDrawer({ item, source, sources, events, evidence, dependencies, aliases, allItems, viewerName, busy, close, transition, note, splitAlias, onItem }: { item: WorkItem; source: Source | null; sources: Source[]; events: WorkEvent[]; evidence: DashboardState["evidence"]; dependencies: DashboardState["dependencies"]; aliases: DashboardState["aliases"]; allItems: WorkItem[]; viewerName: string; busy: boolean; close: () => void; transition: (status: WorkStatus, reason?: string) => void; note: (summary: string) => void; splitAlias: (aliasId: string) => void; onItem: (id: string) => void }) {
