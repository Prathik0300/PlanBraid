...
function PlanDialog({ plan, sources, allItems, events, evidence, dependencies, aliases, close, onItem }: { plan: ExecutionPlan; sources: Source[]; allItems: WorkItem[]; events: WorkEvent[]; evidence: DashboardState["evidence"]; dependencies: DashboardState["dependencies"]; aliases: DashboardState["aliases"]; close: () => void; onItem: (workItemId: string) => void }) {
  ...
  function toggle(id: string) { setExpandedId((current) => current === id ? null : id); setTab("overview"); }
  function row(item: PlanStep | StuckItem, stuck = false) {
    const expanded = item.id === expandedId;
    const corroboration = "corroboration" in item ? item.corroboration : 0;
    return <div className={`plan-step-wrap ${expanded ? "expanded" : ""}`} key={item.id}>
      <button className={`plan-step-row ${stuck ? "stuck" : ""}`} onClick={() => toggle(item.id)} aria-expanded={expanded}>
        ...
        <span className="plan-step-right">
          ...
          <span className="plan-chevron" aria-hidden="true">{expanded ? "⌃" : "⌄"}</span>
        </span>
      </button>
      {expanded && <PlanItemAccordion item={item} source={sources.find((entry) => entry.id === item.sourceId) ?? null} sources={sources} events={events.filter((entry) => entry.workItemId === item.id)} evidence={evidence.filter((entry) => entry.workItemId === item.id)} dependencies={dependencies.filter((entry) => entry.fromWorkItemId === item.id || entry.toWorkItemId === item.id)} aliases={aliases.filter((entry) => entry.workItemId === item.id)} allItems={allItems} tab={tab} setTab={setTab} onItem={onItem} />}
    </div>;
  }
  return <div className="dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
...
function TaskDrawer({ item, source, sources, events, evidence, dependencies, aliases, allItems, viewerName, busy, hideNoteInput, close, transition, note, splitAlias, onItem, linkDependencies }: { item: WorkItem; source: Source | null; sources: Source[]; events: WorkEvent[]; evidence: DashboardState["evidence"]; dependencies: DashboardState["dependencies"]; aliases: DashboardState["aliases"]; allItems: WorkItem[]; viewerName: string; busy: boolean; hideNoteInput?: boolean; close: () => void; transition: (status: WorkStatus, reason?: string) => void; note: (summary: string) => void; splitAlias: (aliasId: string) => void; onItem: (id: string) => void; linkDependencies: (prerequisiteIds: string[]) => void }) {
