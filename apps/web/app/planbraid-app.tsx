...
import { ALLOWED_TRANSITIONS, type Command, type DashboardState, type Notification, type Project, type ProjectMember, type Provider, type Source, type WorkEvent, type WorkItem, type WorkStatus } from "@/lib/contracts";
...
function ProjectRail({ data, avatarUrl, selected, selectedSource, sources, onSelect, onSource, open, toggle, onNew, onProfile, command, busy, onOpenAccountSetup, onManageIntegrations }: { data: DashboardState; avatarUrl: string | null; selected: string; selectedSource: string | null; sources: Source[]; onSelect: (id: string) => void; onSource: (id: string | null) => void; open: boolean; toggle: () => void; onNew: () => void; onProfile: () => void; command: (command: Command, success: string | ((result: CommandResult) => string)) => Promise<CommandResult>; busy: boolean; onOpenAccountSetup: () => void; onManageIntegrations: (project: Project) => void }) {
  ...
  function projectRow(project: Project) {
    ...
    return <div key={project.id} className={`project-row-wrap ${selected === project.id ? "selected" : ""}`}>
      ...
      <ProjectMenu project={project} busy={busy} onRename={() => startRename(project)} onManageTeam={() => setManagingTeamProject(project)} onManageAgents={() => setManagingProject(project)} onManageIntegrations={() => onManageIntegrations(project)} onDelete={() => void command({ action: "delete_project", projectId: project.id, idempotencyKey: requestId("delete-project") }, `Deleted "${project.name}"`)} />
    </div>;
  }
  ...
  return <aside className={`project-rail ${open ? "open" : "collapsed"}`} aria-label="Projects and agent conversations">
    ...
    {managingTeamProject && <TeamManageDialog project={managingTeamProject} members={data.projectMembers.filter((member) => member.projectId === managingTeamProject.id)} items={data.workItems.filter((item) => item.projectId === managingTeamProject.id)} busy={busy} command={command} close={() => setManagingTeamProject(null)} />}
  </aside>;
}

function ProjectMenu({ project, busy, onRename, onManageTeam, onManageAgents, onManageIntegrations, onDelete }: { project: Project; busy: boolean; onRename: () => void; onManageTeam: () => void; onManageAgents: () => void; onManageIntegrations: () => void; onDelete: () => void }) {
  ...
  return <div className="project-menu-wrap">
    ...
    {open && position && createPortal(<div ref={menuRef} className="project-menu" role="menu" style={position}>
      ...
      <button role="menuitem" onClick={() => { setOpen(false); setConfirmingDelete(false); onManageTeam(); }}>Manage project team</button>
...
function TeamManageDialog({ project, members, items, busy, command, close }: { project: Project; members: ProjectMember[]; items: WorkItem[]; busy: boolean; command: (command: Command, success: string) => Promise<CommandResult>; close: () => void }) {
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  return <div className="dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
    <section className="team-manage-dialog" role="dialog" aria-modal="true" aria-labelledby="team-manage-title">
      <header><div><span className="eyebrow">PROJECT TEAM</span><h2 id="team-manage-title">Members of {project.name}</h2><p>Imported members are refreshed from Basecamp or Jira. Local members are assignment choices only and do not receive access to Planbraid.</p></div><button className="icon-button" onClick={close} aria-label="Close">×</button></header>
      <div className="team-member-list">{members.map((member) => { const owned = items.filter((item) => item.assigneeMemberIds.includes(member.id)); const active = owned.filter((item) => ["ready", "in_progress", "blocked", "in_review"].includes(item.status)).length; const done = owned.filter((item) => item.status === "done").length; return <div className="team-member-row" key={member.id}>
        <span className="team-avatar">{member.avatarUrl ? <img src={member.avatarUrl} alt="" /> : member.name.slice(0, 1).toUpperCase()}</span>
        <span><strong>{member.name}</strong><small>{member.title || member.email || (member.provider === "planbraid" ? "Planbraid member" : `Imported from ${member.provider}`)} · {active} active · {done} done</small></span>
        <span className={`team-provider ${member.provider}`}>{member.provider}</span>
        {member.provider === "planbraid" ? <button className="team-remove" disabled={busy} onClick={() => void command({ action: "remove_project_member", projectId: project.id, memberId: member.id, idempotencyKey: requestId("remove-member") }, `${member.name} removed from the project team`)}>Remove</button> : <small>Managed by {member.provider}</small>}
      </div>; })}</div>
      <form className="team-add-form" onSubmit={(event) => { event.preventDefault(); if (!name.trim()) return; void command({ action: "add_project_member", projectId: project.id, name: name.trim(), email: email.trim() || undefined, idempotencyKey: requestId("add-member") }, `${name.trim()} added to the project team`).then(() => { setName(""); setEmail(""); }); }}>
        <label>Name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Team member name" /></label><label>Email <small>optional</small><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@company.com" /></label><button disabled={busy || !name.trim()}>Add member</button>
      </form>
    </section>
  </div>;
}
...
function ProfileDialog({ viewer, projects, initialTab, close, onImported, toast }: { viewer: DashboardState["viewer"]; projects: Project[]; initialTab: "profile" | "integrations"; close: () => void; onImported: () => Promise<void>; toast: (message: string) => void }) {
  ...
  return <div className="dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section className="profile-dialog" role="dialog" aria-modal="true" aria-labelledby="profile-title">
    ...
    {tab === "integrations" ? <IntegrationsPanel projectId={null} projects={projects} onImported={onImported} toast={toast} /> : local ? <div className="profile-local-note"><strong>Local development workspace</strong><p>Account sessions are required on the hosted Planbraid app. Localhost keeps a developer-only workspace so the product can be tested without creating an account.</p></div> : <>
      ...
      <div className="profile-actions"><button className="signout-button" disabled={busy} onClick={() => void signOut()}>Sign out</button><small>Signing out does not disconnect your authorized MCP agents.</small><a className="profile-privacy-link" href="/privacy" target="_blank" rel="noreferrer">Privacy policy</a></div>
    </>}
  </section></div>;
}
...
function OwnerPicker({ item, members, fallback, busy, onChange }: { item: WorkItem; members: ProjectMember[]; fallback: string; busy: boolean; onChange: (memberIds: string[]) => void }) {
  const selected = new Set(item.assigneeMemberIds);
  const label = selected.size ? members.filter((member) => selected.has(member.id)).map((member) => member.name).join(", ") : item.assignee ?? "Unassigned";
  function toggle(memberId: string) {
    const next = new Set(selected);
    if (next.has(memberId)) next.delete(memberId); else next.add(memberId);
    onChange(members.filter((member) => next.has(member.id)).map((member) => member.id));
  }
  return <details className="owner-picker"><summary title={label || fallback}>{label || fallback}</summary><div className="owner-picker-menu">
    <button type="button" disabled={busy || selected.size === 0} onClick={() => onChange([])}>Unassigned</button>
    {members.map((member) => <label key={member.id}><input type="checkbox" disabled={busy} checked={selected.has(member.id)} onChange={() => toggle(member.id)} /><span className="owner-avatar">{member.avatarUrl ? <img src={member.avatarUrl} alt="" /> : member.name.slice(0, 1).toUpperCase()}</span><span><strong>{member.name}</strong><small>{member.title || member.email || (member.provider === "planbraid" ? "Planbraid" : `Imported from ${member.provider}`)}</small></span></label>)}
    {!members.length && <p>No project members yet. Add them from the project menu.</p>}
  </div></details>;
}

function TaskDrawer({ item, members, source, sources, events, evidence, dependencies, aliases, allItems, viewerName, busy, hideNoteInput, transparentBackdrop, close, assignOwners, transition, note, splitAlias, onItem, linkDependencies }: { item: WorkItem; members: ProjectMember[]; source: Source | null; sources: Source[]; events: WorkEvent[]; evidence: DashboardState["evidence"]; dependencies: DashboardState["dependencies"]; aliases: DashboardState["aliases"]; allItems: WorkItem[]; viewerName: string; busy: boolean; hideNoteInput?: boolean; transparentBackdrop?: boolean; close: () => void; assignOwners: (memberIds: string[]) => void; transition: (status: WorkStatus, reason?: string) => void; note: (summary: string) => void; splitAlias: (aliasId: string) => void; onItem: (id: string) => void; linkDependencies: (prerequisiteIds: string[]) => void }) {
  const queryClient = useQueryClient();
