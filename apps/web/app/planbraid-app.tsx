...
function TaskDrawer({ item, source, sources, events, evidence, dependencies, aliases, allItems, viewerName, busy, close, transition, note, splitAlias, onItem, linkDependencies }: { item: WorkItem; source: Source | null; sources: Source[]; events: WorkEvent[]; evidence: DashboardState["evidence"]; dependencies: DashboardState["dependencies"]; aliases: DashboardState["aliases"]; allItems: WorkItem[]; viewerName: string; busy: boolean; close: () => void; transition: (status: WorkStatus, reason?: string) => void; note: (summary: string) => void; splitAlias: (aliasId: string) => void; onItem: (id: string) => void; linkDependencies: (prerequisiteIds: string[]) => void }) {
