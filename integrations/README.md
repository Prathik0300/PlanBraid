...
**Optional: repository symbol grounding (TypeScript/JavaScript only).** Run `npm install` inside `integrations/bridge` to let the hook additionally parse each turn's changed TS/JS files with tree-sitter and report the function, class, method, interface, type, and enum names it finds, alongside repository state. This is entirely opt-in — the bridge itself has zero required dependencies and works exactly as above without this step — and it degrades silently if the packages aren't installed. What leaves the machine is names and file:line locations only, never source text, the same class of information as the changed-file paths already reported. Set `PLANBRAID_DISABLE_SYMBOLS=1` to turn it off even with the packages installed. This grounds a proposal's loose mention of a function against real code instead of a guessed name (RECONCILIATION_ARCHITECTURE.md §7).
